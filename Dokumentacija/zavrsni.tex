...
U slučaju da je u prethodnom koraku točnost pala ispod vrijednosti određene pragom $c$, učit ćemo PGD-om, a inače ćemo učiti s metodom \textit{RandomFGSM} koja predstavlja FGSM metodu s nasumičnom inicijalizacijom.
...
a prema rezultatima rada~\cite{li2020towards} modeli učeni \textit{FastAdvW} metodom postižu bolje rezultate čak i naspram najnovijih varijanta učenja PGD-om, 
...
Sve naučene mreže arhitekture su ResNet-18 i učene su 80 epoha. Jedina iznimka ovome je mreža učena "besplatnim" učenjem. Ta mreža učena je ukupno 10 epoha, ali s hiperparametrom ponavljanja iznosa 8.
Ovime postižemo jednak broj iteracija učenja svih mreža.
% Kako bi sve mreže imale jednak broj iteracija učenja, mreža učena "besplatnim" učenjem učena je ukupno 10 epoha, ali s hiperparametrom ponavljanja iznosa 8. 
Kao mjeru gubitka kod svih mreža korištena je unakrsna entropija, a kao optimizator korišten je stohastički gradijentni spust sa zamahom (engl.\ \textit{momentum}) iznosa 0.9 i propadanjem težina iznosa $5 \cdot 10^{-4}$.
...
Pritom su tijekom učenja PGD metodom neprijateljski primjeri konstruirani koristeći PGD sa 7 iteracija.
...
najbolje performanse ima model učen \textit{FastAdvW} metodom. Njegova točnost na prirodnom skupu za testiranje druga je najviša od svih modela koji nude određenu mjeru robusnosti,
a njegova točnost na neprijateljskim primjerima usporediva je s točnosti modela učenog PGD metodom. 
...
S obzirom na to da \textit{FastAdvW} modeli zadnjih 10 epoha uče isključivo PGD-om, takav rezultat je i očekivan.
Modeli učeni \textit{FastAdv+} metodom dobra su alternativa korištenju \textit{FastAdvW} modela jer se uče kraće i postižu malo bolje rezultate na prirodnom skupu za testiranje.
Uočimo da rano zaustavljanje učenja kod \textit{FastAdv+} i \textit{FastAdvW} modela nema nikakav učinak na performanse. 
Gledajući da je glavni cilj tih metoda rješavanje problema katastrofalne prenaučenosti prisutnog kod \textit{FastAdv} modela, ovakav rezultat je očekivan, 
a govori nam da ove nadogradnje stvarno uspijevaju u svojoj namjeri.

U slučaju da nam je glavni cilj brzo naučiti model koji je barem u nekoj mjeri otporan na neprijateljske primjere, kao najbolja opcija ističe se učenje \textit{FreeAdv} metodom.
Kao što je već rečeno, \textit{FreeAdv} model učen je koristeći ponavljanje (engl.\ \textit{replay}) iznosa 8 pa je stoga ukupno učen 10 epoha - ukupan broj epoha smanjen je za faktor jednak ponavljanju.
...
Nižu točnost na neprijateljskim primjerima tumačimo kao posljedicu učenja na nedovoljno perturbiranim primjerima. 
...
Dobra strana algoritma \textit{FreeAdv} svakako je vrijeme učenja - ovaj model uči se kraće čak i od modela učenog na prirodnom skupu podataka. 
Toliku kratkoću učenja pripisujemo činjenici da je tijekom učenja za algoritam \textit{FreeAdv} provedena evaluacija nakon epoha samo 10 puta, dok je za ostale algoritme evaluacija provedena punih 80 puta.
...
Korištenjem \textit{FastAdv} metode u kombinaciji s ranim zaustavljanjem, točnost na neprijateljskim primjerima podjednaka je s točnosti \textit{FastAdv+} modela,
...
Ako uspoređujemo modele učene \textit{FastAdv}, \textit{FastAdv+} ili \textit{FastAdvW} metodom, najkraće vrijeme učenja ima \textit{FastAdv} model s ranim zaustavljanjem.
Gledajući da \textit{FastAdv+} model pri uočavanju pada točnosti na neprijateljskim primjerima $s$ sljedećih mini-grupa uči PGD-om, 
a \textit{FastAdvW} model uz to zadnjih 10 epoha uči isključivo PGD-om, ovakav rezultat je i očekivan.
...
Slika~\ref{fig:acc_comparison} prikazuje usporedbu točnosti na skupu za testiranje i točnosti na neprijateljskim primjerima konstruiranim iz prirodnog skupa za testiranje koristeći PGD metodu s 20 iteracija.
...
Model učen \textit{Free} metodom ima najnižu točnost na neprijateljskim primjerima, ali zato, kao što smo vidjeli na slici~\ref{fig:train_time_comparison}, ima najkraće vrijeme učenja.
...
Slika~\ref{fig:fast_acc} prikazuje usporedbu točnosti na skupu za učenje i točnosti na skupu za testiranje po epohama za \textit{FastAdv} model.
...
Slika~\ref{fig:fast_adv_acc} prikazuje točnost \textit{FastAdv} modela na neprijateljskim primjerima po epohama.
...
Stupac \textit{Točnost} ponovno označava točnost na prirodnom skupu podataka, a stupac \textit{Točnost, otrov}
...
U ovome radu, napravili smo kratak pregled osnovnih načina za postizanje modela otpornih na neprijateljske primjere. To je učenje FGSM-om ili PGD-om.
...
Korištenjem načina učenja \textit{FastAdvW}, postigli smo modele robusnosti otprilike jednake robusnosti PGD modela, ali s potrebnih skoro dvostruko manje vremena za učenje.
