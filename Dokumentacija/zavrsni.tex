...
\begin{algorithm}
    ...
    \begin{algorithmic}
        \STATE{\textbf{Ulaz:} $x$ -- ulazne slike, $N$ -- broj epoha, $M$ -- veličina skupa za učenje, $\eta$ -- stopa učenja, $\epsilon$ -- ograničenje perturbacije, $\alpha$ -- veličina koraka po iteraciji, $K$ -- broj iteracija za generiranje neprijateljskih primjera}
        \\\hrulefill
        \STATE{$\theta = initializeModelParams()$}
        \FOR{$(ep = 1 \dots N)$}
            \FOR{$(i = 1 \dots M)$}
                \STATE{$x_{adv} = generateAdversarial(x_{i}, \epsilon, \alpha, K)$}
...
Na pseudokodu~\ref{algo:adv_learning} moguće je vidjeti općeniti algoritam za učenje s neprijateljskim primjerima.
Pritom inicijalizaciju parametara modela prikazujemo pozivom funkcije \textit{initializeModelParams}.
