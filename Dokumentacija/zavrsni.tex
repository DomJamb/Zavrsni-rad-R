...
\begin{algorithm}
    \caption{Konstrukcija neprijateljskih primjera s napadom PGD}
    \label{algo:pgd}
    \begin{algorithmic}
...
Pseudokod~\ref{algo:pgd} prikazuje način konstrukcije neprijateljskih primjera s PGD-om. 
...
Općenito govoreći, kako bismo model učili s K-PGD-om, učenje će trajati otprilike K + 1 puta dulje naspram prirodnog učenja~\cite{shafahi2019adversarial}.
...
Uočeno je da modeli učeni s PGD-om imaju visoku robusnost, no, kao što je već rečeno, zahtijevaju puno vremena za učenje.
...
Ponavljanjem mini-grupe nekoliko puta zaredom pokušavamo postići postupak sličan napadu PGD, a da pritom ne platimo veliku dodatnu cijenu. 
...
Za modele učene s FGSM-om dugo se smatralo da nisu otporni na neprijateljske primjere konstruirane iterativnim metodama.
...
Standardno se perturbacija za FGSM metodu s L\textsubscript{$\infty$} ograničenjem inicijalizira na 0 ili na iznos koeficijenta $\epsilon$, bilo s pozitivnim ili negativnim predznakom.
Međutim, pokazuje se da ako za inicijalizaciju iskoristimo nasumičnu inicijalizaciju, učenje s FGSM-om postiže znatno bolju robusnost na iterativne napade.
...
Autori brzog učenja tvrde da njihova metoda nudi gotovo jednaku mjeru robusnosti protiv iterativnih napada kao i modeli učeni s PGD-om, 
ali sa značajno kraćim vremenom učenja~\cite{wong2020fast}. Čak i ako već navedene optimizacije (korištenje cikličke stope učenja i računanja u mješovitoj preciznosti) primijenimo na "besplatno" učenje i učenje s PGD-om, 
...
Taj problem pojavljuje se kod svih postupaka koji uče s FGSM-om, no manje je izražen kod brzog učenja nego kod običnog učenja s FGSM-om.
...
Kao i kod modela učenih s običnim FGSM-om, i modeli učeni s brzim učenjem susreću se s katastrofalnom prenaučenošću, ali se od nje u periodu od nekoliko mini-grupa mogu oporaviti te nastaviti dalje normalno učiti.
...
U slučaju da je u prethodnom koraku točnost pala ispod vrijednosti određene pragom $c$, učit ćemo s PGD-om, a inače ćemo učiti s metodom \textit{RandomFGSM} koja predstavlja FGSM metodu s nasumičnom inicijalizacijom.
...
U originalnome radu~\cite{li2020towards}, za provjeru je korištena PGD metoda, a isto je reproducirano i u ovome radu. 

Ovakav način učenja omogućava nam proizvoljno dugo učenje modela, no i dalje ne dostiže jednaku mjeru robusnosti kao i najnovije varijante učenja s PGD-om.
Kako bismo dodatno unaprijedili \textit{FastAdv+} učenje, predložena je nadogradnja: zadnjih nekolicinu epoha model ćemo učiti isključivo s PGD-om. 
...
a prema rezultatima rada~\cite{li2020towards} modeli učeni s \textit{FastAdvW} metodom postižu bolje rezultate čak i naspram najnovijih varijanta učenja s PGD-om, 
...
a kao optimizator korišten je stohastički gradijentni spust sa zamahom (engl.\ \textit{momentum}) iznosa 0.9 i propadanjem težina iznosa $5 \cdot 10^{-4}$.
...
Sve ostale mreže koristile su stopu učenja s kosinusnim kaljenjem. Pritom je maksimalan broj koraka postavljen na ukupan broj epoha, a iznos stope učenja mijenja se nakon svake epohe.
...
ali i točnost na neprijateljskim primjerima konstruiranim iz prirodnog skupa za testiranje koristeći napad PGD s 20 iteracija. 
...
U tablici~\ref{tbl:mjerenja_robusno_ucenje} stupac \textit{LR} označava osnovnu stopu učenja, stupac \textit{Točnost} točnost na skupu za testiranje, a stupac \textit{20-PGD} točnost na neprijateljskim primjerima konstruiranim iz skupa za testiranje koristeći napad PGD s 20 iteracija.
...
Kada govorimo o robusnim modelima, najvišu točnost na neprijateljskim primjerima ima model učen s PGD-om, no on se i daleko najdulje uči.
...
najbolje performanse ima model učen s \textit{FastAdvW} metodom. Njegova točnost na prirodnom skupu za testiranje druga je najviša od svih modela koji nude određenu mjeru robusnosti,
...
S obzirom na to da \textit{FastAdvW} modeli zadnjih 10 epoha uče isključivo s PGD-om, takav rezultat je i očekivan.
Modeli učeni s \textit{FastAdv+} metodom dobra su alternativa korištenju \textit{FastAdvW} modela jer se uče kraće i postižu malo bolje rezultate na prirodnom skupu za testiranje.
Uočimo da rano zaustavljanje učenja kod \textit{FastAdv+} i \textit{FastAdvW} modela nema nikakav učinak na performanse. 
...
U slučaju da nam je glavni cilj brzo naučiti model koji je barem u nekoj mjeri otporan na neprijateljske primjere, kao najbolja opcija ističe se učenje s \textit{FreeAdv} metodom.
...
Modeli učeni \textit{FastAdv} metodom ograničeni su pojavom katastrofalne prenaučenosti. 
Ovo svojstvo veoma je vidljivo kod \textit{FastAdv} modela bez ranog zaustavljanja - takav model postiže visoku točnost na prirodnom skupu podataka, ali zato nije nimalo otporan na neprijateljske primjere.
Korištenjem \textit{FastAdv} metode u kombinaciji s ranim zaustavljanjem, točnost na neprijateljskim primjerima podjednaka je s točnošću \textit{FastAdv+} modela,
...
Zbog usporedbe, na grafu je prikazano i vrijeme učenja modela učenog na prirodnim podatcima, kao i vrijeme učenja modela učenog s PGD-om.
Kao što je bilo vidljivo i iz tablice~\ref{tbl:mjerenja_robusno_ucenje}, daleko najdulje vrijeme učenja ima model učen s PGD-om.
S druge strane, najkraće vrijeme učenja ima \textit{FreeAdv} model. 
Ako uspoređujemo modele učene s \textit{FastAdv}, \textit{FastAdv+} ili \textit{FastAdvW} metodom, najkraće vrijeme učenja ima \textit{FastAdv} model s ranim zaustavljanjem.
Gledajući da \textit{FastAdv+} model pri uočavanju pada točnosti na neprijateljskim primjerima $s$ sljedećih mini-grupa uči s PGD-om, 
a \textit{FastAdvW} model uz to zadnjih 10 epoha uči isključivo s PGD-om, ovakav rezultat je i očekivan.
...
Model učen s \textit{Free} metodom ima najnižu točnost na neprijateljskim primjerima, ali zato, kao što smo vidjeli na slici~\ref{fig:train_time_comparison}, ima najkraće vrijeme učenja.
...
Ovime potvrđujemo da je stvarno došlo do katastrofalne prenaučenosti. Kako bismo ovo izbjegli, \textit{FastAdv} model možemo učiti s ranim zaustavljanjem. 
...
Kao i prije, svi modeli osim \textit{FastAdv} modela učeni su koristeći stopu učenja s kosinusnim kaljenjem, dok \textit{FastAdv} modeli koriste cikličku stopu učenja.
...
U ovome radu, napravili smo kratak pregled osnovnih načina za postizanje modela otpornih na neprijateljske primjere. To je učenje s FGSM-om ili PGD-om.
...
Implementirali smo i evaluirali točnost, kao i brzinu učenja predloženih nadogradnji na klasične načine učenja s neprijateljskim primjerima: \textit{FreeAdv}, \textit{FastAdv}, \textit{FastAdv+} i \textit{FastAdvW} metoda.
...
Po pitanju detekcije zatrovanih podataka, istraživanje bi trebalo proširiti na dodatne načine učenja s neprijateljskim primjerima poput \textit{FastAdv+} i \textit{FastAdvW} metoda.
...
U slučaju da se pokaže da ovo svojstvo imaju svi robusni modeli učeni na zatrovanom skupu podataka, trebalo bi dodatno istražiti mogućnost primjene ovog svojstva kao detektora postojanja zatrovanih podataka.
...
Uočavamo i istražujemo probleme prisutne pri korištenju PGD načina učenja na kompleksnijim modelima.
