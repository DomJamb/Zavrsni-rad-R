...
\begin{algorithm}
    \caption{Konstrukcija neprijateljskih primjera koristeći napad PGD}
    \label{algo:pgd}
    \begin{algorithmic}
...
Pseudokod~\ref{algo:pgd} prikazuje način konstrukcije neprijateljskih primjera koristeći napad PGD. 
...
Otkriće da se visoka mjera robusnosti može postići i koristeći FGSM s nasumičnom inicijalizacijom perturbacije korišteno je u tzv.\ brzom učenju (engl.\ \textit{fast adversarial training})~\cite{wong2020fast}.
...
Kao i kod modela učenih s običnim FGSM-om, i modeli učeni koristeći brzo učenje susreću se s katastrofalnom prenaučenošću, ali se od nje u periodu od nekoliko mini-grupa mogu oporaviti te nastaviti dalje normalno učiti.
...
Ovo svojstvo veoma je vidljivo kod \textit{FastAdv} modela bez ranog zaustavljanja učenja - takav model postiže visoku točnost na prirodnom skupu podataka, ali zato nije nimalo otporan na neprijateljske primjere.
Korištenjem \textit{FastAdv} učenja u kombinaciji s ranim zaustavljanjem, točnost na neprijateljskim primjerima podjednaka je s točnošću \textit{FastAdv+} modela,
...
Ako uspoređujemo modele učene koristeći \textit{FastAdv}, \textit{FastAdv+} ili \textit{FastAdvW} način učenja, najkraće vrijeme učenja ima \textit{FastAdv} model s ranim zaustavljanjem.
